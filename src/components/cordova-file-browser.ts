...
export let createCordovaFileBrowser = (projector: Projector) => {
  ...
  let headerText = createText({ htmlContent: '<h2>[Cordova] add new file </h2>' });
  let titleTextField = createTextField({ label: 'title' }, { getValue: () => newFileTitle, setValue: (value) => { newFileTitle = value; } });
  let contentTextField = createTextField({ label: 'content' }, { getValue: () => newFileContent, setValue: (value) => { newFileContent = value; } });
  let createFileButton = createButton({ text: 'Create the file', primary: true }, { onClick: createNewFile });
